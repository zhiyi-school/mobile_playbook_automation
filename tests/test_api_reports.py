...
def test_app_risk_history_reads_matching_rows(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(api_app, "REPORTS_ROOT", tmp_path / "reports")
    _write_dashboard(
        api_app.REPORTS_ROOT,
        "2026-01-02_00-00-00",
    ...
    history = api_app.app_risk_history("app_one", "risk_one")
...
def test_app_risk_history_falls_back_to_report_json_verdict(monkeypatch, tmp_path):
    ...
    run_dir = _write_dashboard(api_app.REPORTS_ROOT, row["run_timestamp"], [row])
...
def test_evidence_file_allows_work_and_report_roots(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(api_app, "REPORTS_ROOT", tmp_path / "reports")
    _write_dashboard(api_app.REPORTS_ROOT, "run1", [])
    evidence = tmp_path / "work" / "ios" / "evidence.txt"
    evidence.parent.mkdir(parents=True)
    evidence.write_text("ok")

    response = api_app.evidence_file("run1", "work/ios/evidence.txt")
...
def test_evidence_file_rejects_paths_outside_allowed_roots(monkeypatch, tmp_path):
    ...
    with pytest.raises(HTTPException) as exc_info:
        api_app.evidence_file("run1", str(secret))
